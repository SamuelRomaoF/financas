-- SCRIPT FINAL CORRIGIDO PARA APLICAR TODAS AS MIGRAÇÕES
...
-- 2. ATUALIZAR EMPRÉSTIMOS EXISTENTES
DO $$
DECLARE
    loan_record RECORD;
...
BEGIN
    -- Processar cada empréstimo sem valor de parcelas pagas
...
-- 3. CRIAR/ATUALIZAR FUNÇÕES RPC DO DASHBOARD
...
DECLARE
    ...
    expenses_fn_check BOOLEAN;
    monthly_fn_check BOOLEAN;
BEGIN
    ...
    SELECT EXISTS (
        SELECT 1
        FROM pg_proc
        WHERE proname = 'get_expenses_by_category'
    ) INTO expenses_fn_check;
    
    SELECT EXISTS (
        SELECT 1
        FROM pg_proc
        WHERE proname = 'get_monthly_summary'
    ) INTO monthly_fn_check;
    
    -- Mostrar resultados
    RAISE NOTICE '----------------------------------------';
    RAISE NOTICE 'RESULTADO DAS MIGRAÇÕES:';
    RAISE NOTICE '----------------------------------------';
    RAISE NOTICE 'Coluna paid_installments: %', CASE WHEN column_check THEN 'INSTALADA' ELSE 'FALHOU' END;
    RAISE NOTICE 'Função get_expenses_by_category: %', CASE WHEN expenses_fn_check THEN 'INSTALADA' ELSE 'FALHOU' END;
    RAISE NOTICE 'Função get_monthly_summary: %', CASE WHEN monthly_fn_check THEN 'INSTALADA' ELSE 'FALHOU' END;
    RAISE NOTICE '----------------------------------------';
    RAISE NOTICE 'Migrações concluídas!';
    RAISE NOTICE '----------------------------------------';
END $$; 
-- Versão: 1.0 - 17/06/2025
